...
func LaunchWorkspaceDirectly(ctx context.Context, api *ComponentAPI, opts ...LaunchWorkspaceDirectlyOpt) (*LaunchWorkspaceDirectlyResult, error) {
	...
	if ideImage == "" {
		...
		err := api.client.Resources(api.namespace).List(context.Background(), &pods, func(opts *metav1.ListOptions) {
			opts.LabelSelector = "component=server"
		})
		if err != nil {
			return nil, xerrors.Errorf("cannot find server pod: %q", err)
		}

		config, err := GetServerConfig(api.namespace, api.client)
		if err != nil {
			return nil, err
		}

		ideImage = config.WorkspaceDefaults.IDEImageAliases.Code
	}
...
func LaunchWorkspaceFromContextURL(ctx context.Context, contextURL string, api *ComponentAPI, serverOpts ...GitpodServerOpt) (*protocol.WorkspaceInfo, func(waitForStop bool), error) {
	var defaultServerOpts []GitpodServerOpt
	if api.username != "" {
		defaultServerOpts = []GitpodServerOpt{WithGitpodUser(api.username)}
	}
